// ============================================================
// POST /api/daily-reports/sync-queue
// Syncs offline-captured daily field reports to the database.
//
// Duplicate detection: report_date + project_id uniqueness.
// If a report for the same project/date already exists,
// the item is returned as 'duplicate' and marked synced
// client-side (no data loss, no double-insert).
//
// Guards:
//   - requireAuth
//   - OFFLINE_FIELD_ENABLED flag
//   - project must belong to caller's org
//   - max 50 items per request (denial-of-service protection)
// ============================================================
...
export async function POST(req: NextRequest) {
  ...
  for (const item of items) {
    try {
      ...
      // report_date is required for duplicate detection
      const report_date = payload.report_date as string | undefined
      if (!report_date) {
        results.push({ local_id, status: 'error', error: 'report_date is required' })
        continue
      }
      ...
      if (insertErr) {
        ...
        results.push({ local_id, status: 'created', report_id: newReport.id as string })
      }
