'use client'
// ============================================================
// WeldDetailClient — all interactive logic for the weld detail
// page. Receives server-prefetched data as initialData so the
// first render is instant with no loading skeleton.
// ============================================================
...
const QRCode      = dynamic(() => import('@/components/shared/QRCode').then(m => m.QRCode), { ssr: false })
...
interface WeldDetailClientProps {
  id:          string
  // Server-prefetched weld data — used as React Query initialData
  // so the page renders immediately without a loading spinner.
  initialData: Awaited<ReturnType<typeof import('@/hooks/useWelds').fetchWeld>>
}

export function WeldDetailClient({ id, initialData }: WeldDetailClientProps) {
  const { profile } = useAuth()
  const { data: weld } = useWeld(id, initialData)
  ...
  const timeline      = weld.timeline ?? []
  const projectName   = (weld.projects as { name: string } | null)?.name ?? '—'

  async function submitStatusUpdate() {
    if (!newStatus || !profile) return
    await updateStatus.mutateAsync({ weldId: id, newStatus, notes: statusNotes || undefined })
    ...
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-3">
          <Link href="/welds" className="p-2 rounded-lg text-surface-500 hover:text-surface-300 hover:bg-surface-700 transition-colors mt-0.5">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <div className="flex items-center gap-3 flex-wrap">
              <h1 className="text-2xl font-bold text-surface-50 font-mono">{weld.weld_id_number}</h1>
      ...
      <div className="card p-4">
        <h2 className="text-xs font-semibold text-surface-500 uppercase tracking-wide mb-3">Inspection Pipeline</h2>
          ...
          {(Object.keys(WELD_STATUS_LABELS) as WeldStatus[]).map((s, i, arr) => {
            ...
            return (
              <div key={s} className="flex items-center gap-1">
                ...
                {i < arr.length - 1 && <span className="text-surface-700 text-xs">›</span>}
              </div>
            )
          })}
        </div>
      ...
      <div className="card p-5">
        <h2 className="text-xs font-semibold text-surface-500 uppercase tracking-wide mb-4">Weld Details</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {weld.welder_stamp && (
            <div className="flex items-start gap-2.5">
              <Tag className="w-4 h-4 text-brand-500 mt-0.5 flex-shrink-0" />
              <div><p className="text-xs text-surface-500">Welder Stamp</p><p className="text-sm font-bold text-brand-300 font-mono mt-0.5">{weld.welder_stamp}</p></div>
            </div>
          )}
          {weld.welder_name && (
            <div className="flex items-start gap-2.5">
              <User className="w-4 h-4 text-surface-500 mt-0.5 flex-shrink-0" />
              <div><p className="text-xs text-surface-500">Welder</p><p className="text-sm text-surface-200 mt-0.5">{weld.welder_name}</p></div>
            </div>
          )}
          {weld.weld_date && (
            <div className="flex items-start gap-2.5">
              <Calendar className="w-4 h-4 text-surface-500 mt-0.5 flex-shrink-0" />
              <div><p className="text-xs text-surface-500">Weld Date</p><p className="text-sm text-surface-200 mt-0.5">{formatDate(weld.weld_date)}</p></div>
            </div>
          )}
          {weld.spool_number && (
            <div className="flex items-start gap-2.5">
              <Package className="w-4 h-4 text-surface-500 mt-0.5 flex-shrink-0" />
              <div><p className="text-xs text-surface-500">Spool</p><p className="text-sm font-mono text-surface-200 mt-0.5">{weld.spool_number}</p></div>
            </div>
          )}
          {weld.pipe_size && (
            <div className="flex items-start gap-2.5">
              <Layers className="w-4 h-4 text-surface-500 mt-0.5 flex-shrink-0" />
              <div><p className="text-xs text-surface-500">Pipe Size</p><p className="text-sm text-surface-200 mt-0.5">{weld.pipe_size}</p></div>
            </div>
          )}
          {weld.weld_process && (
            <div className="flex items-start gap-2.5">
              <Wrench className="w-4 h-4 text-surface-500 mt-0.5 flex-shrink-0" />
              <div><p className="text-xs text-surface-500">Process</p><p className="text-sm text-surface-200 mt-0.5">{weld.weld_process}</p></div>
            </div>
          )}
        </div>
        ...
        {weld.notes && (
          <div className="mt-4 pt-4 border-t border-surface-700/60">
            <div className="flex items-start gap-2.5">
              ...
              <div><p className="text-xs text-surface-500 mb-1">Notes</p><p className="text-sm text-surface-300">{weld.notes}</p></div>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="card p-5">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-xs font-semibold text-surface-500 uppercase tracking-wide">Photos &amp; Documentation</h2>
        </div>
      ...
      <div className="card p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            <h2 className="text-xs font-semibold text-surface-500 uppercase tracking-wide">QR Code</h2>
            <p className="text-xs text-surface-600 mt-0.5">Scan to open this weld on any device</p>
          </div>
          <Link href={`/welds/${id}/qr`} className="btn-ghost flex items-center gap-1.5 text-sm">
            <QrCode className="w-3.5 h-3.5" /> Print Sticker
          </Link>
        </div>
        <div className="flex items-start gap-6">
          <div className="bg-white p-3 rounded-xl flex-shrink-0">
            <QRCode value={`${typeof window !== 'undefined' ? window.location.origin : ''}/welds/${id}`} size={120} />
          </div>
      ...
      {/* ── Timeline ── */}
      <div className="card p-5">
        <h2 className="text-xs font-semibold text-surface-500 uppercase tracking-wide mb-4">Status History</h2>
